...
def generar(empleado, contrato, empresa: dict) -> bytes:
    buffer = BytesIO()
    ...
    empresa_nombre  = empresa.get("nombre", "EMPRESA DEMO S.A.S").upper()
    empresa_nit     = empresa.get("nit", "900000000-0")
    empresa_ciudad  = empresa.get("ciudad", "la ciudad")
    empresa_tel     = empresa.get("telefono", "N/A")
    empresa_email   = empresa.get("email", "N/A")
    empresa_dir     = empresa.get("direccion", "N/A")
    responsable     = empresa.get("responsable_nombre", "N/A").upper()
    responsable_cargo = empresa.get("responsable_cargo", "DIRECTOR(A) TALENTO HUMANO").upper()
    ...
    # ── Firma ──────────────────────────────────────────────────────────────────
    firma_data = [[
        Paragraph("_" * 30, _S["firma_c"]),
        Paragraph("", _S["firma_c"]),
    ]]
